...
export default function WineCountryPanel({
  ...
  if (!country) {
    return (
      <section className="mt-6 rounded-[30px] border border-dashed border-rose-100 bg-white/50 p-7 text-center shadow-[0_18px_50px_rgba(35,20,30,0.04)] backdrop-blur-md">
        ...
        <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-neutral-500">
          選取國家後，這裡會顯示該國的酒款數量、平均評分、產區與代表酒款。
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="mt-6 rounded-[30px] border border-white/70 bg-white/62 p-6 shadow-[0_18px_50px_rgba(35,20,30,0.055)] backdrop-blur-md">
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-end gap-3">
            ...
            <span className="mb-1 rounded-full bg-rose-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-rose-400">
              {country.code}
            </span>
          </div>

          <p className="mt-2 text-sm text-neutral-400">{country.name}</p>

          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-[22px] border border-rose-100/70 bg-white/72 p-4 shadow-sm">
          ...
          <div className="mt-4 rounded-[22px] border border-rose-100/70 bg-white/72 p-4 shadow-sm">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-neutral-400">
              Regions
            </p>
        ...
        <div className="rounded-[26px] border border-rose-100/70 bg-white/72 p-5 shadow-sm">
          ...
          <div className="mt-4 divide-y divide-rose-100/70">
            {recentWines.length > 0 ? (
              recentWines.map((wine) => (
                <div key={wine.id} className="py-4 first:pt-0 last:pb-0">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h5 className="font-bold text-neutral-950">
                        {wine.name}
                      </h5>
                    ...
                    {typeof wine.rating === "number" && (
                      <span className="rounded-2xl bg-neutral-950 px-3 py-2 text-sm font-bold text-white">
