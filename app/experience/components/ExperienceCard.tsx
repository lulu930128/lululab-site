...
export default function ExperienceCard({ item, type, onClick }: CardProps) {
  ...
  // 狀態膠囊樣式：對應 Projects 的 System Active 標籤
  const statusStyle = "flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-bold text-emerald-600 border border-emerald-100";
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-[#eCeAf4] bg-white/80 p-6 shadow-sm transition hover:-translate-y-2 hover:shadow-xl backdrop-blur-md">
      <div className="relative mb-6 h-52 w-full overflow-hidden rounded-2xl bg-neutral-100">
      ...
      <div className="flex-1 px-1">
        <div className="flex justify-between items-start">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8a79da]">{item.developer}</p>
          <span className="font-mono text-[9px] font-bold text-neutral-300">ID: {item.id}</span>
        </div>
        <h3 className="mt-1 text-xl font-bold text-neutral-900 line-clamp-1 group-hover:text-[#7a6ccf] transition-colors">{item.title}</h3>
      ...
      <button 
        ...
        className="mt-6 w-full rounded-2xl border border-neutral-100 bg-white py-3 text-[11px] font-bold text-neutral-700 transition hover:bg-[#f4f1fb] hover:text-[#7a6ccf] hover:border-[#7a6ccf]/20 uppercase tracking-widest shadow-sm"
      >
        View Review
      </button>
    </article>
